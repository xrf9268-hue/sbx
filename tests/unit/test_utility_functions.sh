...
test_get_file_mtime_function() {
    ...
    # Test 2: Timestamp format - Linux (YYYY-MM-DD HH:MM:SS) or macOS (Mon D HH:MM:SS YYYY)
    # Accept either format for cross-platform compatibility
    local linux_format='^[0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2}$'
    local macos_format='^[A-Z][a-z]{2} +[0-9]{1,2} [0-9]{2}:[0-9]{2}:[0-9]{2} [0-9]{4}$'
    if [[ "$mtime" =~ $linux_format ]] || [[ "$mtime" =~ $macos_format ]]; then
