...
# Retry configuration (immutable defaults, can be overridden via environment at startup)
# Per Google SRE best practices: configuration should be immutable after initialization
# Custom values should be passed as function parameters, not by modifying global state
declare -gr RETRY_MAX_ATTEMPTS="${RETRY_MAX_ATTEMPTS:-3}"
declare -gr RETRY_BACKOFF_BASE="${RETRY_BACKOFF_BASE:-2}"
declare -gr RETRY_BACKOFF_MAX="${RETRY_BACKOFF_MAX:-32}"
declare -gr RETRY_JITTER_MAX="${RETRY_JITTER_MAX:-1000}" # milliseconds

# Global retry budget (prevent retry amplification)
declare -gr GLOBAL_RETRY_BUDGET="${GLOBAL_RETRY_BUDGET:-30}"
...
#   $2 - (optional) base backoff multiplier, defaults to RETRY_BACKOFF_BASE
#   $3 - (optional) max backoff seconds, defaults to RETRY_BACKOFF_MAX
#
# Returns:
#   Backoff time in milliseconds
#
# Example:
#   backoff_ms=$(calculate_backoff 2)        # Uses defaults
#   backoff_ms=$(calculate_backoff 2 3 60)   # Custom: base=3, max=60
calculate_backoff() {
  local attempt="$1"
  local base="${2:-${RETRY_BACKOFF_BASE}}"
  local max="${3:-${RETRY_BACKOFF_MAX}}"
...
# Provides fine-grained control over retry behavior without modifying global state
# Per Google SRE: use function parameters instead of mutable global configuration
#
# Arguments:
#   $1 - max attempts
...
retry_with_custom_backoff() {
  ...
  while [[ ${attempt} -lt ${max_attempts} ]]; do
    ...
    # Calculate backoff time with custom parameters (no global state modification)
    local backoff_ms=0
    backoff_ms="$(calculate_backoff "${attempt}" "${backoff_base}" "${backoff_max}")"
