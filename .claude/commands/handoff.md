...
**sbx-lite specific:** Pay special attention to Reality protocol configurations, bash coding standards, TDD practices, and backward compatibility requirements.
...
1. Chronologically analyze each message and section of the conversation. For each section thoroughly identify:
   ...
   - **sbx-lite:** Reality protocol considerations (short IDs, TLS nesting, validation)
   - **sbx-lite:** Code quality aspects (strict mode, constants extraction, ShellCheck)
...
### sbx-lite Specific Sections (Add when relevant):

8. **Testing Context** (if applicable):
   - Unit tests affected or created
   - Integration tests required
   - Test status (passing/failing/pending)
   - Coverage considerations

9. **Code Quality Standards** (if applicable):
   - ShellCheck status and warnings
   - Strict mode compliance (`set -euo pipefail`)
   - Constants extracted (magic numbers → named constants)
   - Helper functions used or created

10. **Reality Protocol Context** (if applicable):
    - Short ID considerations (8-char limit)
    - Configuration structure (`tls.reality` nesting)
    - Validation steps (`sing-box check` status)
    - SNI selection rationale

11. **Backward Compatibility** (if applicable):
...
* Fix Issue #42
* Fix Reality Protocol Validation
...
## 5. Pending Tasks
[List of tasks explicitly requested by user]

## 6. Current Work
[Detailed description of work in progress immediately before handoff]
